...
const { getTokenWithUserData, processUserDataCiphered } = require("./tokenUser.helper");

async function validarToken(db, token) {
   // 1. Buscar el token (el campo 'token' no está cifrado)
   const tokenData = await getTokenWithUserData(db, token);
   ...
   if (tokenData.usuario) {
      response.data.usuario = processUserDataCiphered(tokenData.usuario);
   }
