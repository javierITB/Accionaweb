...
const RequestDetails = ({ request, isVisible, onClose, onSendMessage, onUpdate, endpointPrefix = "respuestas" }) => {
   ...
   const isClosed = ["finalizado", "archivado", "rechazado"].includes(status);
...
Máximo permitido: ${MAX_CLIENT_FILES} archivos.`;
   ...
   const handleUploadAdditionalFiles = async () => {
      ...
      try {
         ...
         await fetchClientSignatures(request._id);
   ...
   const handleDownloadClientSignature = async (signedFile, index) => {
      ...
      try {
         ...
         setIsUploading(false);
         setIsDownloadingSignedPdf(null);
      }
   };

   const handleRemoveFile = (index) => {
      setClientSelectedAdjuntos((prev) => prev.filter((_, i) => i !== index));
   };
...
Máximo permitido: ${MAX_FIRMADO_CLIENT_FILES} archivos.`;
   ...
   const handleRemoveFileSignedSelected = (index) => {
      setSelectedSignedFiles((prev) => prev.filter((_, i) => i !== index));
   };
   ...
   const renderSignedDocumentSection = () => {
      ...
      if (notShowSignedSection) return null;
      ...
      return (
         <div>
            ...
            <div className="bg-muted/50 rounded-lg  px-4 space-y-4">
                  ...
                  <div className="space-y-3 ">
                     {signedPdfFiles.map((signedFile, index) => (
                        ...
                        >
                           <div className="flex items-center space-x-3">
                              <Icon
                                 name={getMimeTypeIcon(signedFile?.clientSignedPdf?.mimeType)}
                                 size={20}
                              ...
                              <div>
                                 ...
                                 <p className="text-xs text-muted-foreground">
                                    ...
                                    {formatDate(signedFile?.clientSignedPdf?.uploadedAt)}
                                 </p>
                              </div>
                           </div>
   ...
   return (
      <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
         <div
            className="bg-card border border-border flex flex-col shadow-brand-active w-full overflow-hidden rounded-none h-[clamp(500px,85dvh,800px)] max-h-[calc(100dvh-2rem)] sm:max-w-4xl sm:rounded-lg"
            ref={containerRef}
         >
                  ...
                  <div className="flex items-center space-x-3">
                     ...
                     {(clientSelectedAdjuntos?.length > 0 || selectedSignedFiles?.length > 0) && (
                        ...
                        >
                           {isUploading
                              ? "Subiendo..."
                              : `Subir archivos (${(clientSelectedAdjuntos?.length || 0) + (selectedSignedFiles?.length || 0)})`}
                        </Button>
                     )}
