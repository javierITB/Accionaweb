  ...
  //  const cargo = sessionStorage.getItem("cargo");
   ...
   const validateFiles = (files, question) => {
      ...
      for (let i = 0; i < files.length; i++) {
         ...
         if (!isTypeValid) {
            const allowedTypesText = allowedTypesArray
               .map((type) => {
         ...
         questionList.forEach((question, index) => {
            ...
            if (hasValidAnswer) {
               if (question.type === "file" && answer instanceof FileList) {
                  mappedAnswers[questionTitle] = Array.from(answer)
                     .map((file) => file.name)
                     .join(", ");
               } else {
   ...
   const getFileTypesDescription = (question) => {
      ...
      const allowedTypes = question.accept
         .split(",")
         .map((type) => type.trim())
         .filter((type) => type.length > 0);
      ...
      return (
         <div className="space-y-3">
            ...
            >
               ...
               <p className="text-sm text-gray-500 mt-1">Haz clic para seleccionar un archivo</p>
            </button>
            ...
            <div className="space-y-1">
               ...
               <p className="text-xs text-gray-500">
                  ...
                  {isMultipleAllowed ? `Máximo ${maxFiles} archivos, uno por uno` : "1 archivo solamente"}
               </p>
               <p className="text-xs text-gray-500">
                     ...
                     {selectedFiles.map((file, index) => (
                        ...
                        >
                           <div className="flex items-center space-x-3 flex-1 min-w-0">
                              <Icon name={getFileIcon(file.type)} size={16} className="text-gray-400 flex-shrink-0" />
   ...
   const renderQuestion = (question, index, showNumber = true, parentPath = "") => {
      ...
      const baseInputClass =
         "w-full px-3 py-3 sm:py-2 border border-gray-300 rounded-md bg-white text-black text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all";
                           ...
                           <div key={idx} className="space-y-2">
                              <label className="flex items-center space-x-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50">
                                 <input
                                    ...
                                    checked={isChecked}
                                    onChange={(e) =>
                                       handleRadioChange(question.id, getQuestionTitle(question), e.target.value)
                                    }
                           ...
                           <div key={idx} className="space-y-2">
                              <label className="flex items-center space-x-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50">
                                 <input
                                    ...
                                    checked={isChecked}
                                    onChange={() =>
                                       handleCheckboxChange(question.id, getQuestionTitle(question), optionText)
                                    }
                  ...
                  <div className="flex items-start space-x-3">
                     ...
                     <div className="flex-1 min-w-0">
                        ...
                        {question?.description && (
                           <p className="text-gray-600 text-sm sm:text-base break-words">{question?.description}</p>
                        )}
                     </div>
                  </div>
               </div>

               <div className={showNumber ? "ml-0 sm:ml-11" : ""}>{renderInput()}</div>
            </div>
         </div>
      );
   };
   ...
   const handleSubmit = async () => {
      ...
      try {
         ...
         const cleanAnswers = Object.fromEntries(
            Object.entries(processedAnswers).filter(
               ([_, value]) =>
         ...
         if (Object.values(answers).some((answer) => answer instanceof FileList)) {
            ...
            if (todosLosArchivos.length > 0) {
               ...
               for (let i = 0; i < todosLosArchivos.length; i++) {
                  ...
                  try {
                     console.log(
                        `Enviando archivo ${i + 1} de ${todosLosArchivos.length}:`,
                        archivoData.adjunto.fileName,
                     );
   ...
   if (!formData?.title && formData?.questions?.length === 0) {
      return (
         <div className="text-center py-8 sm:py-12">
            ...
            <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">Formulario no disponible</h3>
