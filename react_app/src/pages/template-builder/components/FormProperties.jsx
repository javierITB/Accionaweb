...
const TemplateList = ({ onUpdateFormData }) => {
  ...
  const fetchFormsAndTemplates = async () => {
    try {
      ...
      // Cargar formularios
      const formsRes = await fetch('https://back-vercel-iota.vercel.app/api/forms');
      if (!formsRes.ok) throw new Error('Error al obtener formularios');
      const formsData = await formsRes.json();

      // Cargar plantillas
      const templatesRes = await fetch('https://back-vercel-iota.vercel.app/api/plantillas');
  ...
  const handleDeleteTemplate = async (formId, plantillaId) => {
    ...
    try {
      ...
      const response = await fetch(`https://back-vercel-iota.vercel.app/api/plantillas/${plantillaId}`, {
        method: 'DELETE',
      });
  ...
  const handleDuplicateFromTemplate = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://back-vercel-iota.vercel.app/api/plantillas/${selectedTemplateForDuplicate.formId}`);
