...
const TicketConfig = ({ userPermissions = {} }) => {
   // Estado del Sidebar
   ...
   const canAccess = userPermissions.includes("view_configuracion_tickets");
   ...
   const handleDeleteCategory = async () => {
      ...
      if (
         !confirm(`¿Estás SEGURO de eliminar la categoría "${editingCategory.name}"? Esta acción no se puede deshacer.`)
      )
         return;
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
               <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="space-y-4">
                           {subcategoriesToEdit.length > 0 ? (
                              subcategoriesToEdit.map((sub, index) => (
                                 <div
                                    key={index}
                                    className="flex gap-4 p-4 border border-border rounded-lg bg-muted/20 items-end"
                                 >
                     ...
                     <details className="group" open>
                        ...
                        <div className="space-y-4">
                           {statusesToEdit.map((status, index) => (
                              <div
                                 key={index}
                                 className="flex flex-col md:flex-row gap-4 p-4 border border-border rounded-lg bg-muted/20 items-end md:items-center"
                              >
                                 <div className="flex-1 w-full">
                                    ...
                                    <div className="relative">
                                       ...
                                       >
                                          <div className="flex items-center gap-2 max-w-[85%]">
                                             {status.icon ? (
                                                <Icon name={status.icon} size={16} className="shrink-0" />
                                             ) : (
                                             ...
                                             <span className="truncate">
                                                {iconOptions.find((o) => o.value === status.icon)?.label ||
                                                   status.icon ||
                                                   "Sin icono"}
                                             </span>
                                          </div>
                                    ...
                                    <div className="relative">
                                       ...
                                       >
                                          <div className="flex items-center gap-2">
                                             <div
                                                className={`w-4 h-4 rounded-full ${getColorPreviewClass(status.color)}`}
                                             ></div>
                                             <span className="truncate">
                                             ...
                                             >
                                                <div className="flex flex-col max-h-60 overflow-y-auto bg-popover">
                                                   {colorOptions.map((option) => (
                                                      ...
                                                      >
                                                         <div
                                                            className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-white shadow-sm ${getColorPreviewClass(option.value)}`}
                                                         >
                                                            {status.icon ? (
                                                               <Icon name={status.icon} size={16} strokeWidth={3} />
                                                            ) : (
                  ...
                  <div className="p-6 border-t border-border bg-muted/10 flex justify-between gap-3">
                     {!editingCategory.isNew && permissions.delete ? (
                        <Button
                           variant="ghost"
