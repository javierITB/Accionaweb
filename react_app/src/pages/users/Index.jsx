...
const FormReg = () => {
  ...
  const sortedUsers = useMemo(() => {
    ...
    if (filters.field && filters.value) {
      processData = processData.filter(u => String(u[filters.field]).toLowerCase() === String(filters.value).toLowerCase());
    }
    if (searchTerm.trim() !== '') {
      const term = searchTerm.toLowerCase();
      processData = processData.filter(u => Object.values(u).some(v => String(v).toLowerCase().includes(term)) || `${u.nombre} ${u.apellido}`.toLowerCase().includes(term));
    }
    if (sortConfig.key) {
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            {filters.field && <button onClick={() => setFilters({ field: null, value: null })} className="text-xs bg-primary/10 text-primary px-3 py-1 rounded-full">Filtro: {filters.field} ✕</button>}
          </div>
          <div className="relative w-full md:w-80">
            <Icon name="Search" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-zinc-500" />
            <input type="text" placeholder="Búsqueda rápida..." className="w-full pl-10 pr-10 py-2.5 bg-card text-foreground border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-foreground transition-colors"><Icon name="X" size={14} /></button>}
          </div>
        </div>
            ...
            <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
              <tr>
                {['nombre', 'empresa', 'mail', 'cargo', 'rol', 'estado', 'createdAt'].map(k => (
                  <th key={k} className="px-4 py-3 text-left cursor-pointer hover:bg-muted" onClick={() => handleSort(k)}>
