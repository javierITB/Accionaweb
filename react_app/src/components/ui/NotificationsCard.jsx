...
const NotificationsCard = ({ user, onUnreadChange }) => {
   ...
   useEffect(() => {
      const fetchNotifications = async () => {
         try {
            const res = await fetch(`${API_BASE_URL}/noti/${user}`);
   ...
   const handleDeleteNotifications = async () => {
      try {
         ...
         const res = await fetch(`${API_BASE_URL}/noti/${mail}`, {
            method: "DELETE",
            headers: {
   ...
   const handleDeleteNotification = async (id) => {
      try {
         ...
         const res = await fetch(`${API_BASE_URL}/noti/${mail}/${id}`, {
            method: "DELETE",
            headers: {
               "Content-Type": "application/json",
            },
   ...
   const markAllAsRead = async () => {
      try {
         ...
         const res = await fetch(`${API_BASE_URL}/noti/${mail}/leido-todas`, {
            method: "PUT",
         ...
         setNotifications((prev) => prev.map((n) => ({ ...n, isRead: true, leido: true })));

         setFilteredNotifications((prev) => prev.map((n) => ({ ...n, isRead: true, leido: true })));
   ...
   const handleNotificationClick = (notification) => {
      ...
      // 1️⃣ Actualización optimista inmediata
      setNotifications((prev) => prev.map((n) => (n.id === notiId ? { ...n, isRead: true } : n)));

      setFilteredNotifications((prev) => prev.map((n) => (n.id === notiId ? { ...n, isRead: true } : n)));

      // 2️⃣ Navegar inmediatamente (sin esperar al backend)
      if (notification?.actionUrl) {
         navigate(notification.actionUrl);
      }

      // 3️⃣ Llamar al backend en segundo plano (sin await)
      if (mail && notiId) {
         fetch(`${API_BASE_URL}/noti/${mail}/${notiId}/leido`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
         ...
         <div className="sticky top-0 bg-card z-10 pt-4 px-4 pb-3 border-b border-border">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-1 justify-center">
               {notificationTypes.map(
                ...
                ${activeFilter === type ? "text-white shadow-sm" : "bg-muted text-muted-foreground hover:bg-muted-hover"}
              `}
                           style={{
                ...
                ${activeFilter === type ? "bg-white/30 text-white" : "bg-black/10 text-foreground"}
              `}
                           >
                              {countByType[type] || 0}
                           </span>
                  ...
                  >
                     ...
                     <div className="flex items-start">
                        <div className="flex-1 min-w-0">
                           ...
                           <p className="text-sm text-muted-foreground pr-10">{notification?.message}</p>
                        </div>
                     </div>
                  </div>
               ))
